...
export class LeadModel {
    ...
    static async findAll(limit = 50, offset = 0) {
        const result = await query('SELECT * FROM leads ORDER BY created_at DESC LIMIT $1 OFFSET $2', [limit, offset]);
        return result.rows;
    }
    ...
    static async findByUserOrClient(userId, clientId) {
        if (clientId) {
            ...
            const result = await query('SELECT * FROM leads WHERE assigned_to = $1 OR client_id = $2 ORDER BY created_at DESC', [userId, clientId]);
            return result.rows;
        }
        else {
            // Get only leads assigned to this user
            const result = await query('SELECT * FROM leads WHERE assigned_to = $1 ORDER BY created_at DESC', [userId]);
            return result.rows;
        }
    }
    // Get leads by assigned user
    static async findByAssignedTo(assignedTo) {
        const result = await query('SELECT * FROM leads WHERE assigned_to = $1 ORDER BY created_at DESC', [assignedTo]);
        return result.rows;
    }
    ...
    static async findByAssignedToPaginated(assignedTo, limit, offset) {
        ...
        const result = await query('SELECT * FROM leads WHERE assigned_to = $1 ORDER BY created_at DESC LIMIT $2 OFFSET $3', [assignedTo, limit, offset]);
        return { leads: result.rows, total };
    }
    // Get leads by client
    static async findByClientId(clientId) {
        const result = await query('SELECT * FROM leads WHERE client_id = $1 ORDER BY created_at DESC', [clientId]);
        return result.rows;
    }
