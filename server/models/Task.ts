import { query } from '../database/connection.js';

export interface Task {
  id: number;
  title: string;
  description?: string;
  status: 'ממתין' | 'בביצוע' | 'הושלם';
  priority: 'נמוך' | 'בינוני' | 'גבוה';
  due_date: Date; // Will contain both date and time
  assigned_to?: number | null;
  lead_id?: number;
  customer_id?: number;
  created_by: number;
  created_at: Date;
  updated_at: Date;
  notified: boolean;
}

export interface CreateTaskData {
  title: string;
  description?: string;
  status?: 'ממתין' | 'בביצוע' | 'הושלם';
  priority?: 'נמוך' | 'בינוני' | 'גבוה';
  due_date: Date; // Will contain both date and time
  assigned_to?: number | null;
  lead_id?: number;
  customer_id?: number;
  created_by: number;
}

export class TaskModel {
  // Create a new task
  static async create(taskData: CreateTaskData): Promise<Task> {
    const { 
      title,
      description,
      status = 'ממתין',
      priority = 'בינוני',
      due_date,
      assigned_to,
      lead_id,
      customer_id,
      created_by
    } = taskData;
    
    const result = await query(
      `INSERT INTO tasks (title, description, status, priority, due_date, assigned_to, lead_id, customer_id, created_by)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
       RETURNING 
         id, title, description, status, priority, 
         TO_CHAR(due_date, 'YYYY-MM-DD"T"HH24:MI:SS') as due_date,
         assigned_to, lead_id, customer_id, created_by, 
         created_at, updated_at, notified`,
      [title, description, status, priority, due_date, assigned_to, lead_id, customer_id, created_by]
    );
    
    return result.rows[0];
  }

  // Find task by ID
  static async findById(id: number): Promise<Task | null> {
    const result = await query(
      `SELECT 
        id, title, description, status, priority, 
        TO_CHAR(due_date, 'YYYY-MM-DD"T"HH24:MI:SS') as due_date,
        assigned_to, lead_id, customer_id, created_by, 
        created_at, updated_at, notified
      FROM tasks WHERE id = $1`,
      [id]
    );
    
    return result.rows[0] || null;
  }

  // Get all tasks with pagination
  static async findAll(limit = 50, offset = 0): Promise<Task[]> {
    const result = await query(
      `SELECT 
        id, title, description, status, priority, 
        TO_CHAR(due_date, 'YYYY-MM-DD"T"HH24:MI:SS') as due_date,
        assigned_to, lead_id, customer_id, created_by, 
        created_at, updated_at, notified
      FROM tasks 
      ORDER BY due_date ASC, created_at DESC 
      LIMIT $1 OFFSET $2`,
      [limit, offset]
    );
    
    return result.rows;
  }

  // Update task
  static async update(id: number, updates: Partial<Task>): Promise<Task | null> {
    const fields: string[] = [];
    const values: any[] = [];
    let paramCount = 1;

    for (const key of Object.keys(updates)) {
      if (key !== 'id' && updates[key as keyof Task] !== undefined) {
        fields.push(`${key} = $${paramCount}`);
        values.push(updates[key as keyof Task]);
        paramCount++;
      }
    }

    if (fields.length === 0) return null;

    values.push(id);
    const result = await query(
      `UPDATE tasks SET ${fields.join(', ')}, updated_at = CURRENT_TIMESTAMP
       WHERE id = $${paramCount}
       RETURNING 
         id, title, description, status, priority, 
         TO_CHAR(due_date, 'YYYY-MM-DD"T"HH24:MI:SS') as due_date,
         assigned_to, lead_id, customer_id, created_by, 
         created_at, updated_at, notified`,
      values
    );

    return result.rows[0] || null;
  }

  // Delete task
  static async delete(id: number): Promise<boolean> {
    const result = await query(
      'DELETE FROM tasks WHERE id = $1',
      [id]
    );
    
    return result.rowCount > 0;
  }

  // Get tasks by status
  static async findByStatus(status: string): Promise<Task[]> {
    const result = await query(
      `SELECT 
        id, title, description, status, priority, 
        TO_CHAR(due_date, 'YYYY-MM-DD"T"HH24:MI:SS') as due_date,
        assigned_to, lead_id, customer_id, created_by, 
        created_at, updated_at, notified
      FROM tasks WHERE status = $1 ORDER BY due_date ASC`,
      [status]
    );
    
    return result.rows;
  }

  // Get tasks by assigned user
  static async findByAssignedTo(assignedTo: number): Promise<Task[]> {
    const result = await query(
      `SELECT 
        id, title, description, status, priority, 
        TO_CHAR(due_date, 'YYYY-MM-DD"T"HH24:MI:SS') as due_date,
        assigned_to, lead_id, customer_id, created_by, 
        created_at, updated_at, notified
      FROM tasks WHERE assigned_to = $1 ORDER BY due_date ASC`,
      [assignedTo]
    );
    
    return result.rows;
  }

  // Get tasks by lead ID
  static async findByLeadId(leadId: number): Promise<Task[]> {
    const result = await query(
      `SELECT 
        id, title, description, status, priority, 
        TO_CHAR(due_date, 'YYYY-MM-DD"T"HH24:MI:SS') as due_date,
        assigned_to, lead_id, customer_id, created_by, 
        created_at, updated_at, notified
      FROM tasks WHERE lead_id = $1 ORDER BY due_date ASC`,
      [leadId]
    );
    
    return result.rows;
  }

  // Get tasks by customer ID
  static async findByCustomerId(customerId: number): Promise<Task[]> {
    const result = await query(
      `SELECT 
        id, title, description, status, priority, 
        TO_CHAR(due_date, 'YYYY-MM-DD"T"HH24:MI:SS') as due_date,
        assigned_to, lead_id, customer_id, created_by, 
        created_at, updated_at, notified
      FROM tasks WHERE customer_id = $1 ORDER BY due_date ASC`,
      [customerId]
    );
    
    return result.rows;
  }

  // Get overdue tasks
  static async findOverdue(): Promise<Task[]> {
    const result = await query(
      `SELECT 
        id, title, description, status, priority, 
        TO_CHAR(due_date, 'YYYY-MM-DD"T"HH24:MI:SS') as due_date,
        assigned_to, lead_id, customer_id, created_by, 
        created_at, updated_at, notified
      FROM tasks WHERE due_date < CURRENT_TIMESTAMP AND status != $1 ORDER BY due_date ASC`,
      ['הושלם']
    );
    
    return result.rows;
  }

  // Get tasks due today
  static async findDueToday(): Promise<Task[]> {
    const result = await query(
      `SELECT 
        id, title, description, status, priority, 
        TO_CHAR(due_date, 'YYYY-MM-DD"T"HH24:MI:SS') as due_date,
        assigned_to, lead_id, customer_id, created_by, 
        created_at, updated_at, notified
      FROM tasks 
      WHERE DATE(due_date) = CURRENT_DATE AND status != $1 
      ORDER BY due_date ASC`,
      ['הושלם']
    );
    
    return result.rows;
  }

  // Mark task as notified
  static async markAsNotified(id: number): Promise<boolean> {
    const result = await query(
      'UPDATE tasks SET notified = true WHERE id = $1',
      [id]
    );
    
    return result.rowCount > 0;
  }

  // Get tasks by user or client (for data segregation)
  static async findByUserOrClient(userId: number, clientId?: number): Promise<Task[]> {
    if (clientId) {
      // Get tasks assigned to user or from leads/customers belonging to their client
      const result = await query(
        `SELECT t.* FROM tasks t
         LEFT JOIN leads l ON t.lead_id = l.id
         LEFT JOIN customers c ON t.customer_id = c.id
         WHERE t.assigned_to = $1 OR l.client_id = $2 OR c.client_id = $2
         ORDER BY t.due_date ASC`,
        [userId, clientId]
      );
      return result.rows;
    } else {
      // Get only tasks assigned to this user
      const result = await query(
        'SELECT * FROM tasks WHERE assigned_to = $1 ORDER BY due_date ASC',
        [userId]
      );
      return result.rows;
    }
  }
}
