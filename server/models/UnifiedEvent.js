...
export class UnifiedEventModel {
    ...
    static async getEventsByUser(userId) {
      ...
      SELECT 
        ...
        end_time as "endTime",
        advance_notice::integer as "advanceNotice",
    ...
    `, [userId]);
        ...
        const events = result.rows.map((row) => ({
            ...row,
            // Fix: Ensure advanceNotice is properly converted to number
            advanceNotice: row.advanceNotice ? parseInt(row.advanceNotice) : 0,
            // Ensure times are in UTC ISO format with Z suffix for consistent parsing
    ...
    static async getEventsByType(userId, eventType) {
      ...
      ORDER BY start_time ASC
    `, [userId, eventType]);
        // Convert Date objects to strings to avoid timezone issues
        const events = result.rows.map((row) => ({
            ...row,
            // Fix: Ensure advanceNotice is properly converted to number
            advanceNotice: row.advanceNotice ? parseInt(row.advanceNotice) : 0,
    ...
    `, [userId]);
        ...
        const events = result.rows.map((row) => ({
            ...
            // Fix: Ensure advanceNotice is properly converted to number
            advanceNotice: row.advanceNotice ? parseInt(row.advanceNotice) : 0,
            // Ensure UTC times are properly formatted with Z suffix
            startTime: row.startTime ? row.startTime.toISOString() : null,
            endTime: row.endTime ? row.endTime.toISOString() : null,
            createdAt: row.createdAt ? row.createdAt.toString() : null,
            updatedAt: row.updatedAt ? row.updatedAt.toString() : null
    ...
    static async createEvent(userId, eventData) {
        ...
        // Parse the simple time strings as local time - handle both ISO and simple formats
        ...
        if (eventData.startTime.includes('T')) {
            ...
            // Simple format like "2024-01-15T14:30:00" - parse directly
            startTime = new Date(eventData.startTime);
        }
        
        if (eventData.endTime.includes('T')) {
            // ISO format - parse as local time
        ...
        console.log('🔍 Parsed times:', { startTime, endTime, advanceNotice: eventData.advanceNotice });
      ...
      INSERT INTO unified_events (
        ...
        start_time as "startTime",
        end_time as "endTime",
        advance_notice::integer as "advanceNotice", is_active as "isActive",
    ...
    `, [
            ...
            startTime,
            endTime,
            eventData.advanceNotice !== undefined ? eventData.advanceNotice : (eventData.eventType === 'reminder' ? 1440 : 0),
        ...
        return {
            ...event,
            // Fix: Ensure advanceNotice is properly converted to number
            advanceNotice: event.advanceNotice ? parseInt(event.advanceNotice) : 0,
            // Ensure times are in UTC ISO format with Z suffix for consistent parsing
