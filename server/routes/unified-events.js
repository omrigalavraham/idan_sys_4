...
export function createUnifiedEventsRoutes() {
    ...
    router.get('/health', (req, res) => {
        res.json({ status: 'ok', service: 'unified-events' });
    });
    ...
    router.get('/notifications', authenticateToken, async (req, res) => {
        try {
            ...
            console.log('🔍 Server: getEventsForNotification result:', events.length, 'events');
            res.json({ events });
        }
        catch (error) {
    ...
    // Debug endpoint to check all reminders
    // @ts-ignore
    router.get('/debug/reminders', authenticateToken, async (req, res) => {
        try {
            const userId = parseInt(req.user.id);
            const events = await UnifiedEventModel.getEventsByType(userId, 'reminder');
            console.log('🔍 Server: Debug - all reminders for user:', userId, 'count:', events.length);
            res.json({ 
                userId,
            ...
            res.status(500).json({ error: 'שגיאה בבדיקת תזכורות' });
        }
    });
    ...
    router.post('/', authenticateToken, async (req, res) => {
        try {
            ...
            // Validate event type
            const validTypes = ['reminder', 'meeting', 'lead', 'task', 'no-reminder'];
            if (!validTypes.includes(eventData.eventType)) {
                return res.status(400).json({
                    error: 'סוג אירוע לא תקין. סוגים מותרים: reminder, meeting, lead, task, no-reminder'
                });
            }
