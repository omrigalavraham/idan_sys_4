...
const parseQueryInt = (value: any, defaultValue: number) => {
...
router.get('/', authenticateToken, async (req: any, res: Response, next: NextFunction) => {
  try {
    ...
    const client_id = req.query.client_id ? parseInt(req.query.client_id as string as string) : undefined;
...
router.post('/', authenticateToken, async (req: any, res: Response, next: NextFunction) => {
  try {
    ...
    if (authReq.user.role === 'agent') return res.status(403).json({ error: 'Agents cannot create users' });
    if (authReq.user.role === 'manager' && role !== 'agent') return res.status(403).json({ error: 'Managers can only create agents' });
    if (!password) return res.status(400).json({ error: 'Password is required' });

    const password_hash = await bcrypt.hash(password, 12);

    let assigned_manager_id: number | undefined;
...
router.put('/:id', authenticateToken, async (req: any, res: Response, next: NextFunction) => {
  try {
    ...
    if (!isOwnProfile) {
      if (authReq.user.role === 'agent') return res.status(403).json({ error: 'Agents can only update their own profile' });
      if (authReq.user.role === 'manager') {
        const targetUser = await UserModel.findById(targetUserId);
        if (!targetUser || targetUser.role !== 'agent') return res.status(403).json({ error: 'Managers can only update agents' });
      }
    }

    if (role && !isOwnProfile) {
      if (authReq.user.role === 'manager' && role !== 'agent') return res.status(403).json({ error: 'Managers can only assign agent role' });
    }
...
router.delete('/:id', authenticateToken, async (req: any, res: Response, next: NextFunction) => {
  try {
    ...
    if (authReq.user.role === 'agent') return res.status(403).json({ error: 'Agents cannot delete users' });
    if (authReq.user.role === 'manager') {
      const targetUser = await UserModel.findById(targetUserId);
      if (!targetUser || targetUser.role !== 'agent') return res.status(403).json({ error: 'Managers can only delete agents' });
    }

    if (authReq.user.id === targetUserId) return res.status(400).json({ error: 'Cannot delete your own account' });
...
router.get('/agents/by-manager', authenticateToken, async (req: any, res: Response, next: NextFunction) => {
  try {
    const authReq = req as AuthenticatedRequest;
    if (!authReq.user) {
      return res.status(401).json({ error: 'Unauthorized' });
...
// @ts-ignore
router.post('/cleanup-deleted', authenticateToken, async (req: any, res: Response, next: NextFunction) => {
  try {
    const authReq = req as AuthenticatedRequest;
...
// @ts-ignore
router.get('/pending-deletion-count', authenticateToken, async (req: any, res: Response, next: NextFunction) => {
  try {
    const authReq = req as AuthenticatedRequest;
    if (!authReq.user || authReq.user.role !== 'admin') {
      return res.status(403).json({ error: 'Admin access required' });
...
// @ts-ignore
router.get('/deleted', authenticateToken, async (req: any, res: Response, next: NextFunction) => {
  try {
    const authReq = req as AuthenticatedRequest;
    if (!authReq.user || authReq.user.role !== 'admin') {
      return res.status(403).json({ error: 'Admin access required' });
...
// @ts-ignore
router.delete('/:id/soft', authenticateToken, async (req: any, res: Response, next: NextFunction) => {
  try {
    const authReq = req as AuthenticatedRequest;
    if (!authReq.user || authReq.user.role !== 'admin') {
      return res.status(403).json({ error: 'Admin access required' });
...
// @ts-ignore
router.get('/:id/call-history', authenticateToken, async (req: any, res: Response, next: NextFunction) => {
  try {
    const userId = parseInt(req.params.id as string);
...
// @ts-ignore
router.put('/:id/call-history', authenticateToken, async (req: any, res: Response, next: NextFunction) => {
  try {
    const userId = parseInt(req.params.id as string);
    const requestingUser = req.user;
...
// @ts-ignore
router.get('/profile/:userId', authenticateToken, async (req: any, res: Response, next: NextFunction) => {
...
// Update user profile
// @ts-ignore
router.put('/profile/:userId', authenticateToken, async (req: any, res: Response, next: NextFunction) => {
  try {
    const targetUserId = parseInt(req.params.userId as string);
    const authReq = req as AuthenticatedRequest;
...
// @ts-ignore
router.patch('/:id/status', authenticateToken, async (req: any, res: Response, next: NextFunction) => {
  try {
    const userId = parseInt(req.params.id as string);
