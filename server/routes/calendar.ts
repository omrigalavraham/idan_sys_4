  ...
  try {
    ...
    if (req.user.role === 'admin') {
      ...
      if (start_date && end_date) {
        events = await CalendarEventModel.findByDateRange(
          new Date(start_date as string),
          new Date(end_date as string)
        );
      } else if (event_type) {
        events = await CalendarEventModel.findByType(event_type as string);
      } else {
        events = await CalendarEventModel.findAll(parseInt(limit as string as string), parseInt(offset as string as string));
      }
    } else {
...
router.get('/:id', authenticateToken, async (req: Request & {user?: any}, res: Response) => {
  try {
    const { id } = req.params;
    const event = await CalendarEventModel.findById(parseInt(id as string));
...
router.put('/:id', authenticateToken, async (req: Request & {user?: any}, res: Response) => {
  try {
    ...
    const event = await CalendarEventModel.update(parseInt(id as string), updates);
...
router.delete('/:id', authenticateToken, async (req: Request & {user?: any}, res: Response) => {
  try {
    const { id } = req.params;
    const deleted = await CalendarEventModel.delete(parseInt(id as string));
...
router.get('/lead/:leadId', authenticateToken, async (req: Request & {user?: any}, res: Response) => {
  try {
    ...
    const events = await CalendarEventModel.findByLeadId(parseInt(leadId as string));
    res.json({ events });
  } catch (error) {
...
router.get('/customer/:customerId', authenticateToken, async (req: Request & {user?: any}, res: Response) => {
  try {
    ...
    const events = await CalendarEventModel.findByCustomerId(parseInt(customerId as string));
    res.json({ events });
  } catch (error) {
...
// Get events by user
router.get('/user/:userId', authenticateToken, async (req: Request & {user?: any}, res: Response) => {
  try {
    const { userId } = req.params;
    const events = await CalendarEventModel.findByCreatedBy(parseInt(userId as string));
    res.json({ events });
  } catch (error) {
...
// Get today's events
router.get('/today/all', authenticateToken, async (req: Request & {user?: any}, res: Response) => {
  try {
