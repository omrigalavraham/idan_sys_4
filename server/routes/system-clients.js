...
router.get('/', withAuth(async (req, res) => {
    try {
        ...
        if (active_only === 'true') {
            ...
            clients = await SystemClientModel.findAll(parseInt(limit), parseInt(offset));
        }
...
router.get('/:id', withAuth(async (req, res) => {
    try {
        const { id } = req.params;
        const client = await SystemClientModel.findById(parseInt(id));
        if (!client) {
...
router.delete('/:id', withAuth(async (req, res) => {
    try {
        ...
        const { id } = req.params;
        const deleted = await SystemClientModel.softDelete(parseInt(id));
        if (!deleted) {
...
router.patch('/:id/activate', withAuth(async (req, res) => {
    try {
        ...
        const { id } = req.params;
        const client = await SystemClientModel.activate(parseInt(id));
        if (!client) {
            return res.status(404).json({ error: 'System client not found' });
        }
        res.json({ client });
...
router.patch('/:id/deactivate', withAuth(async (req, res) => {
    try {
        ...
        const { id } = req.params;
        const client = await SystemClientModel.deactivate(parseInt(id));
        if (!client) {
            return res.status(404).json({ error: 'System client not found' });
        }
        res.json({ client });
...
router.get('/:id/config', withAuth(async (req, res) => {
    try {
        const { id } = req.params;
        const config = await SystemClientModel.getConfiguration(parseInt(id));
        if (!config) {
            return res.status(404).json({ error: 'System client not found' });
        }
        res.json({ config });
...
router.put('/:id/config', withAuth(async (req, res) => {
    try {
        ...
        const client = await SystemClientModel.updateConfiguration(parseInt(id), {
            lead_statuses,
            customer_statuses,
            payment_statuses,
            features,
...
// Get client features
router.get('/:id/features', withAuth(async (req, res) => {
    try {
        const { id } = req.params;
        const config = await SystemClientModel.getConfiguration(parseInt(id));
        if (!config) {
...
router.put('/:id/features', withAuth(async (req, res) => {
    try {
        ...
        const client = await SystemClientModel.updateConfiguration(parseInt(id), { features });
        if (!client) {
            return res.status(404).json({ error: 'System client not found' });
        }
...
router.get('/:id/workflow', withAuth(async (req, res) => {
    try {
        const { id } = req.params;
...
router.put('/:id/workflow', withAuth(async (req, res) => {
    try {
...
router.get('/:id/templates', withAuth(async (req, res) => {
    try {
        const { id } = req.params;
        const config = await SystemClientModel.getConfiguration(parseInt(id));
        if (!config) {
...
router.put('/:id/templates', withAuth(async (req, res) => {
    try {
        ...
        const client = await SystemClientModel.updateConfiguration(parseInt(id), { message_templates: templates });
        if (!client) {
            return res.status(404).json({ error: 'System client not found' });
        }
...
router.post('/cleanup-deleted', withAuth(async (req, res) => {
    try {
        if (req.user.role !== 'admin') {
...
router.get('/pending-deletion-count', withAuth(async (req, res) => {
    try {
        if (req.user.role !== 'admin') {
            return res.status(403).json({ error: 'Admin access required' });
        }
...
router.delete('/:id/soft', withAuth(async (req, res) => {
    try {
        if (req.user.role !== 'admin') {
            return res.status(403).json({ error: 'Admin access required' });
        }
