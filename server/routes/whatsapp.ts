 ...
 */
const requireManager = async (req: Request, res: Response, next: Function) => {
  try {
 ...
 */
const requireManagerOrAgent = async (req: Request, res: Response, next: Function) => {
  try {
    const token = req.headers.authorization?.replace('Bearer ', '');
    if (!token) {
      return res.status(401).json({ error: 'No token provided' });
 ...
 */
router.post('/connect', validateInput, requireManager, async (req: Request, res: Response) => {
  try {
 ...
 */
router.put('/update', validateInput, requireManager, async (req: Request, res: Response) => {
  try {
    const user = (req as AuthenticatedRequest).user;
    const validatedData = connectWhatsAppSchema.parse(req.body);
    const { access_token, phone_number_id, business_account_id, app_id, webhook_verify_token } = validatedData;
 ...
 * שליחת הודעות ווטסאפ
 */
router.post('/send', validateInput, requireManagerOrAgent, async (req: Request, res: Response) => {
  try {
    ...
    for (const phoneNumber of phone_numbers) {
      try {
        ...
        const response = await axios.post(
          `https://graph.facebook.com/v18.0/${connection.wa_phone_number_id}/messages`,
          messageData,
          {
            headers: {
 ...
 * בדיקת סטטוס החיבור
 */
router.get('/status', requireManagerOrAgent, async (req: Request, res: Response) => {
  try {
    const user = (req as AuthenticatedRequest).user;
 ...
 * ניתוק חיבור ווטסאפ
 */
router.delete('/disconnect', requireManager, async (req: Request, res: Response) => {
  try {
    const user = (req as AuthenticatedRequest).user;
