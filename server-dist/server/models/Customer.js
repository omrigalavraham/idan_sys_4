...
export class CustomerModel {
    ...
    static async create(customerData) {
        ...
        const { lead_id, created_by, client_id, full_name, phone, email, status = 'פעיל', address, company_name, company_id, vat_number, assigned_rep, payment_status = 'ממתין לתשלום', billing_frequency = 'חד פעמי', total_amount = 0, start_date, payment_plan, payment_type = 'amount', payment_value = 0, payment_vat_included = false, tags = [], custom_fields = {}, notes } = cleanData;
    ...
    static async createFromLead(leadId, customerData) {
        ...
        try {
            ...
            await query('UPDATE leads SET status = $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2', ['לקוח קיים', leadId]);
            await query('COMMIT');
            return customer;
        }
        catch (error) {
    ...
    static async addPayment(customerId, paymentData) {
        const result = await query(`INSERT INTO payments (customer_id, total_amount, payment_status, start_date, installments, installment_amount, payment_type, payment_value, vat_included, notes)
