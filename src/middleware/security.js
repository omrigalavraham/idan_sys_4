...
import { SecurityLogger, SecureSession } from '../utils/security.ts';
...
export const validateSession = (req, res, next) => {
    ...
    try {
        ...
        if (!session.validateSession(sessionId, fingerprint, req.ip)) {
            throw new Error('Invalid session');
        }
        // Update session with current IP
        session.updateSession(sessionId, req.ip);
        next();
    }
    catch (error) {
        const message = error instanceof Error ? error.message : String(error);
